...
interface PrayerEngagementData {
  activePrayerWarriors?: number;
  supportProvided?: number;
  prayerEngagementRate?: number;
  totalPrayers?: number;
  membersWhoPrayedForOthers?: Array<{
    userId: string;
    name: string;
    email: string;
...
interface DevotionalCompletionData {
  message?: string;
...
export function AdminAnalyticsDashboard() {
  ...
  const { data: prayerData, isLoading: prayerLoading } = useQuery<PrayerEngagementData>({
    queryKey: ['/api/admin/analytics/prayer-engagement'],
    enabled: !!user
  });

  const { data: devotionalData, isLoading: devotionalLoading } = useQuery<DevotionalCompletionData>({
